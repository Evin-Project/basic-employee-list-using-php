...
<body>
    <div class="container">
        ...
        <form action="addAction.php" method="post" name="add">
            <div id="employee-form-container">
                <!-- Initial employee form (no remove button here) -->
                <div class="employee-form mb-4" id="employee1">
                    <h4>Employee 1</h4>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Add a new employee form (with a remove button)
            addMoreButton.addEventListener('click', function() {
                employeeCount++;
                const newEmployeeForm = `
                    <div class="employee-form mb-4" id="employee${employeeCount}">
                        ...
                        <!-- Remove button for additional employees -->
                        <button type="button" class="btn btn-danger remove-employee">Remove Employee</button>
                    </div>
                `;
                container.insertAdjacentHTML('beforeend', newEmployeeForm);

                // Attach the event listener to the new "Remove" button
                attachRemoveEmployeeListener();
            });

            // Function to remove employee form
            function attachRemoveEmployeeListener() {
                const removeButtons = document.querySelectorAll('.remove-employee');
                removeButtons.forEach(button => {
                    button.removeEventListener('click', handleRemoveEmployee); // Prevent multiple bindings
                    button.addEventListener('click', handleRemoveEmployee);
                });
            }

            function handleRemoveEmployee(event) {
                const employeeForm = event.target.closest('.employee-form');
                employeeForm.remove();
            }
        });
    </script>
